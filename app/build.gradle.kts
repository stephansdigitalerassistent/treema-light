...
if (!project.hasProperty("usePrebuiltRust")) {
    configure<org.mozilla.rust.android.gradle.RustAndroidPluginExtension> {
        ...
        features = mutableSetOf("uniffi") // features is usually a Set<String> or MutableSet
        // defaultAnd(arrayOf("uniffi")) // This helper might not be easily accessible dynamically. 
        // explicit features list is safer if 'defaultAnd' is an extension function I can't reach easily.
        // Actually, let's keep it simple first. "features" in the DSL usually accepts list/set. 
        // If "defaultAnd" is a method on the extension, we can call it.
        // Check if I can use dynamic lookup.
    }
    // Rewriting the cargo block to use 'configure<...>' style because 'cargo' accessor is missing.
    // The extension class is visible if I import it, or I can use dynamic dispatch with 'extensions.findByName("cargo")?.withGroovyBuilder { ... }' or 'configure("cargo") { ... }' logic.
    // simpler:
    extensions.configure("cargo") {
        val extension = this as org.mozilla.rust.android.gradle.RustAndroidPluginExtension
        extension.prebuiltToolchains = true
        extension.targetDirectory = File("$projectDir/build/generated/source/libthreema")
        extension.module = "$projectDir/../domain/libthreema"
        extension.libname = "libthreema"
        extension.profile = "release"
        extension.pythonCommand = "python3"
        extension.targets = listOf("x86_64", "arm64", "arm", "x86")
        // extension.features ... 'defaultAnd' is a method on the extension class.
        // public void defaultAnd(String[] features)
        extension.features(closureOf<NamedDomainObjectContainer<*>> { 
             // This is getting complicated to reverse engineer the DSL. 
             // The RustAndroidPluginExtension probably has a method `features` or property. 
        })
        // Workaround: The DSL `features { ... }` configures a named domain object container or similar? 
        // No, looking at source, `features` is a property or method.
        // The DSL usually uses `features { ... }`.
        
        // Let's try to just leave the original `cargo` block wrapped in `if` and hope that applying the plugin dynamically
        // doesn't break the script compilation if the accessor is generated by `alias(...) apply false`.
        // Gradle 8+ with version catalogs might generate the accessor even if apply is false?
        // Reference: https://docs.gradle.org/current/userguide/kotlin_dsl.html#type_safe_accessors
        // "Type-safe accessors are available for plugins applied ... in the plugins block"
        // If applied with `apply false`, the accessor IS available? 
        // "Plugins applied with apply false are visible on the build script classpath..."
        // BUT "The type-safe accessors for dependencies contributed by that plugin are NOT available."
        // What about the validation? "Function invocation 'project(...)' expected" was purely about `project`.
        
        // HYPOTHESIS: If I just move the check OUTSIDE `plugins {}` and use `alias(...) apply false` inside, 
        // AND then `if (!prop) apply(plugin=...)`, maybe the original `cargo {}` block works IF the accessor is generated.
        // If not, I'll get an "Unresolved reference: cargo" error.
        
        // Let's try the Minimal Fix first: 
        // 1. `alias(...) apply false` in plugins.
        // 2. `if checking` outside plugins to apply it.
        // 3. Keep `cargo {}` block as is (wrapped in if).
    }
}
